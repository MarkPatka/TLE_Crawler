...
public static class TleExtensions
{
    ...
    {
        var persistedHashes = new HashSet<ReadOnlyMemory<byte>>(alreadyPersisted
            .Select(t => new ReadOnlyMemory<byte>(t.Hash)), new ByteArrayComparer());

        return [.. filteredResponse.Where(tle => !persistedHashes.Contains(tle.Hash.AsMemory()))];
